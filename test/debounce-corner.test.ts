...
describe("LastWinsAndCancelsPrevious — debounce (corner-cases)", () => {
  ...
  it("run → abort → run: первая отменена, вторая выполняется", async () => {
    ...
    const p1 = queue
      .run(1)
      .catch((err) => expect(err).toBeInstanceOf(TaskAbortedError));
    await wait(10);
    queue.abort();
    await wait(10);
  ...
  it("run → run → abort → run: первые две отменены, третий стартует", async () => {
    ...
    const p1 = queue
      .run(1)
      .catch((err) => expect(err).toBeInstanceOf(TaskIgnoredError));
    const p2 = queue
      .run(2)
      .catch((err) => expect(err).toBeInstanceOf(TaskAbortedError));
    await wait(10);
  ...
  it("debounce: аргументы всегда актуальны для последнего run", async () => {
    ...
    queue.run(10).catch((err) => expect(err).toBeInstanceOf(TaskIgnoredError));
    queue.run(20).catch((err) => expect(err).toBeInstanceOf(TaskIgnoredError));
    queue.run(30);
    await wait(21);
